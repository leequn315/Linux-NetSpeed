...
sh_ver="1.3.2"
github="raw.githubusercontent.com/leequn315/Linux-NetSpeed/master"
...
startbbrmod_nanqinlang(){
	...
	if [[ "${release}" == "centos" ]]; then
		yum install -y make gcc
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/leequn315/Linux-NetSpeed/master/bbr/centos/tcp_nanqinlang.c
		...
		mkdir bbrmod && cd bbrmod
		wget -N --no-check-certificate https://raw.githubusercontent.com/leequn315/Linux-NetSpeed/master/bbr/tcp_nanqinlang.c
		echo "obj-m := tcp_nanqinlang.o" > Makefile
		make -C /lib/modules/$(uname -r)/build M=`pwd` modules CC=/usr/bin/gcc-4.9
...
startlotserver(){
	...
	bash <(wget --no-check-certificate -qO- https://raw.githubusercontent.com/leequn315/lotServer/master/Install.sh) install
...
remove_all(){
	...
	if [[ -e /appex/bin/lotServer.sh ]]; then
		bash <(wget --no-check-certificate -qO- https://github.com/leequn315/lotServer/raw/master/Install.sh) uninstall
	fi
	clear
	echo -e "${Info}:清除加速完成。"
	sleep 1s
